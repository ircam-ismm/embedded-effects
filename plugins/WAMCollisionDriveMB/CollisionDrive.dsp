...
noiseGate = _ <: _*((((abs(_) : si.smoo) < tLinear) : si.smooth(pole)) ) : v1 with{
    threshold = hslider("Gate[name:Gate][style:knob]", -15, -30, -10, 1);
    tLinear = threshold : ba.db2linear;
    pole = 0.001 : ba.tau2pole; // att
    v1(x) = attach(x, envelop(x) );
...
capset = (table, int(p)):rdtable * 1e-6 with {
    ...
    p = hslider("attack[name:Attack][style:knob]", 3, 0, 5, 0.1); 
};
...
p2 = ts9 with {
    LogPot(a, x) = ba.if(a, (exp(a * x) - 1) / (exp(a) - 1), x);
    R1 = 1000;
    R2 = 10000 + 500000 * (hslider("drive[name:Drive][style:knob]", 0.1, 0, 1, 0.01) : LogPot(3));
    C = capset;
    a1 = (R1 + R2) * C * 2 * ma.SR;
    a2 = R1 * C * 2 * ma.SR;
    B0 = (1 + a1) / (1 + a2);
...
process1 = noiseGate : p1 : p2 : p3 : *(gain) with {
    gain = hslider("level[name:Level][style:knob]", -10, -20, 4, 0.1) : ba.db2linear : si.smooth(0.993);
};
